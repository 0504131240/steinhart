...
הנתונים נשמרים ב-Firebase Firestore. חלק מהפיצ'רים (התראות פוש, גיבוי יומי, תזכורת חובות שבועית) רצים כפונקציות שרת ב-Vercel וב-Firebase Cloud Functions.
...
### 5. GitHub Secret — לדיפלוי אוטומטי של Firebase Functions

ב-GitHub repo → Settings → Secrets and variables → Actions, להוסיף `FIREBASE_SERVICE_ACCOUNT` עם תוכן קובץ ה-service account key (JSON) שהורדתם משלב 4.

### 6. EmailJS (אופציונלי — לתזכורות חוב באימייל)
...
אייקון ולוגו זמניים (ריבוע כחול עם "S") הוכנו אוטומטית כדי לא להעתיק תמונות אישיות מאתר משפחה אחרת. אפשר להחליף את `icon.jpg` ו-`logo.jpg` בתמונה/לוגו האמיתיים של המשפחה בכל שלב.
