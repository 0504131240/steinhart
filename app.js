...
async function save(){
  ...
  try{
    ...
    await setDoc(doc(db,'appData','familyPayments'),{families,events,fund,goalFunds,adminPass,messages,calItems,birthdays,paymentClaims,globalSettled,visits,notifications,polls,countdowns,yahrzeits});
...
async function load(){
  ...
  try{
    ...
    if(_kidsMigrated||_bdaysCleared)save();
...
async function registerFCMToken(){
  ...
  if(!_fcmForegroundBound){
    ...
    msgMod.onMessage(messaging,payload=>{
      ...
      showNotif(n.title||'Steinhart',n.body||'');
    });
  }
}
...
// different events are netted against each other.
...
// Minimal set of transfers to settle every open event at once, computed from
// each family's combined net position rather than one event in isolation —
// so a debt in one event that's already covered by a credit in another never
// shows up as needing a (redundant) payment, and money routes directly
// between the two families that actually end up owing each other instead of
// chaining through whoever happens to link the two events.
...
// The global transfer list, scoped down to whichever transfers involve at
// least one of this event's own participants — what an event's own window
// should show instead of that event's isolated (non-netted) transfers.
function evGlobalTransfers(ev){
  return calcGlobalTransfers().filter(t=>ev.participants.includes(t.fromFid)||ev.participants.includes(t.toFid));
}
  ...
  const settleLines=transfers.map(t=>{
    return`<div class="settle-row" style="display:flex;align-items:center;justify-content:space-between;gap:6px;flex-wrap:wrap;padding:6px 0">
      ...
      <div style="display:flex;gap:6px;flex-shrink:0">
        <button style="padding:5px 10px;border-radius:6px;border:none;background:var(--blue-mid);color:#fff;font-size:11px;font-weight:700;font-family:var(--font);cursor:pointer;white-space:nowrap" onclick="openPartPayModal(null,'${esc(t.from)}',${t.fromFid},'${esc(t.to)}',${t.toFid},${t.amt},false,false)">✓ שולם</button>
      </div>
    </div>`;
  }).join('');
...
function evFamTransfers(ev,famId){
  ...
  evGlobalTransfers(ev).forEach(t=>{
