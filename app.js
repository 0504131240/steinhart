...
function addNotif(icon,text,pushTarget,hiddenFromFamIds,kind,relatedFamIds){
  ...
  _sendPush(icon+' Steinhart',text,pushTarget,[...(hiddenFromFamIds||[]),...emailOptedFamIds],kind,relatedFamIds);
}
...
function _sendCategoryEmails(icon,text,kind,hiddenFromFamIds,relatedFamIds){
  ...
  families.forEach(f=>{
    ...
    sendEmailNotif([{email:f.email,email2:f.email2,name}],icon+' '+text+' · Steinhart',text,html);
  });
