...
// evShares/evCost/evAdjBalance/famWeight/itemShareFor/hebrewToGregorian/
// kidAge/famAge3PlusChildCount below are copies of the same-named functions
// in app.js. If the calculation logic changes there (split methods,
// settled-entry matching, pot handling, per-item split overrides, the
// under-3 child exclusion...), update these too or the weekly debt email
// will quietly report the wrong amount. (Same warning as the old
// functions/index.js this was ported from.)
const FAM_ADULTS = 2;
...
// Per-capita/weighted splits only count kids old enough to "cost" like a
// person (age 3+, or unknown age) — a baby under 3 is excluded.
...
function itemShareFor(ev, it, fid, families) {
...
function evShares(ev, families) {
  ...
  items.forEach(it => {
    ...
    ev.participants.forEach(fid => {
      const s = itemShareFor(ev, it, fid, families);
